...
def cloud_receive(name, env, in_pipe, out_pipe2):
    ...
    msg_receieved1 = yield in_pipe.get()
    print('%s receives the following message at time %d: %s'  % (name, env.now,msg_receieved1))
    ...
    if chaCha20_poly1305_authenticate_only(integrity_key,byte_msg['nonce'], byte_msg['ciphertext'], byte_msg['tag'],byte_msg['header']):
        ...
        print('%s sends the following message at time %d: %s' % (name, env.now, msg_receieved1)+"\n")
        out_pipe2.put(msg_receieved1)
    else:
        SIM_TIME = 0
        print("Message not verified. Message discarded")
        

def modifying_authentication_tag(name, env2, in_pipe2, out_pipe3):
    yield env2.timeout(40)
...
env2.process(modifying_authentication_tag('During transmission', env2, pipe2, pipe3))
